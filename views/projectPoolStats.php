...
foreach ($this->view->accounts as $account) {
	...
	if (isset($this->view->projStats[$account->getId()])) {
		if (isset($this->view->projStats[$account->getId()]['hostCount'])) {
			$hostCount += $this->view->projStats[$account->getId()]['hostCount'];
			$mag += $this->view->projStats[$account->getId()]['mag'];
		}
	}
	...
	$projects .= '
		<tr>
			...
			<td style="text-align:center;">
				<span class="fa-stack poolAttach">
					<i class="fa fa-circle text-'.($account->attach?'success':'danger').' fa-stack-2x"></i>
					<strong class="fa-stack-1x" style="color:white;"></strong>
				</span>
			</td>
