		...
		if ($this->memberValid) {
			...
			$hostDao->save($this->host);
 			...
 			foreach ($this->xml->project as $project) {
 				...
 				$weakKey = '';
 				if ($account) {
 					$keyObj = $keyDao->getWithAccount($account->getId());
 					if ($keyObj) {
 				...
 				if ($account != null && $account->getId() != 0 && (String)$project->account_key != "" && (String)$project->account_key == $weakKey) { // key indicates if using pool account
	 				...
	 				if ($obj == null) {
	 					$obj = $hostProjectDao->getWithMemberIdAndCpidAndAccountId($this->member->getId(),$cpid,$account->getId());
	 					if ($obj == null) {
	...
	public function getResponseXml() {
		...
		if ($this->memberValid) {
			...
			foreach ($hostProjects as $hostProject) {
				...
				if ($account) {
					$weakKey = '';
					$key = $keyDao->getWithAccount($account->getId());
					if ($key) {
						$weakKey = $key->getWeak();
					}
