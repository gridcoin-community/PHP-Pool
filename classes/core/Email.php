...
class Email {
	...
	public static function getVerificationMessage($key,$userid) {
		return '
			...
			Thank you for taking the time to verify your email address. This will help to keep '.Constants::BOINC_POOL_NAME.' running smoothly and keep you informed of very important updates. 
	...
	function send() {
		...
		$email['custom_headers'] = array();
		if ($this->replyTo) {
			array_push($email['custom_headers'],array(
					'header' => 'Reply-To',
					'value' => $this->replyTo[0]
			));
		}
