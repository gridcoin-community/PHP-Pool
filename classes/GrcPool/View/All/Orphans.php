...
class GrcPool_View_All_Orphans_DAO extends GrcPool_View_All_Orphans_MODELDAO {
	...
	public function getAvailableOrphanPayoutsForPool($minAmount) {
		$sql = 'select * from '.$this->getFullTableName().' where (mag = 0.00 or owed >= '.$minAmount.') and (memberIdPayout != 0 or memberIdCredit != 0) order by owed desc';
