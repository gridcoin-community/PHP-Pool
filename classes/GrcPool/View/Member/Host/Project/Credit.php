...
class GrcPool_View_Member_Host_Project_Credit_DAO extends GrcPool_View_Member_Host_Project_Credit_MODELDAO {
	...
	public function getOwedForPool($min = 0) {
		return $this->fetchAll(array($this->where('owed',$min,'>')));
	}
	...
	public function getTopHosts($limit) {
		$sql = 'select memberId,username,hostId,sum(mag) as magTotal from '.$this->getFullTableName().' group by memberId,username,hostId order by magTotal desc limit '.$limit;
		return $this->query($sql);
	}
