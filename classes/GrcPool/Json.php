...
class GrcPool_Json {
	...
	public static function getHostSettings(GrcPool_Member_OBJ $user,GrcPool_Member_Host_OBJ $host) {
		...
		foreach ($accounts as $idx => $account) {
			...
			if (isset($accountKeys[$account->getId()]) && $accountKeys[$account->getId()]['weak'] != '' && ($account->getWhiteList() && $account->getAttachable() && $accountKeys[$account->getId()]['attachable'])) {
			...
			foreach ($hostProjects as $proj) {
				if ($account->getId() == $proj->getAccountId() && $proj->getAttached() != 2) {
